...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        *   4. 결합도를 낮춰 유지보수성을 증가시킬 수 있다.  -> 결합도 : 한군데만 고치면 다른곳은 안고쳐 된다.
        *  */
        ...
        rabbit.달리기();
        rabbit.울기();
        rabbit.점프();              // 토끼 부모의 메소드 뿐 아니라 본인의 메소드도 가졌다.
        ...
        *   호랑이도 마찬가지로 호랑이 타입이기도 하면서 동물 타입니다.
        * */

        /* 목차. 5. 부모 타입(동물) 으로 자식 인스턴스(토끼, 호랑이) 주소값 저장 */

        동물 a1 = new 토끼();
        동물 a2 = new 호랑이();

        // IS_A 관계 -> 토끼는 동물 / 동물은 다 토끼?
//        토끼 r1 =new 동물();      // 오류 발생 (동물이 더 크기 떄문에)
//        호랑이 t1 = new 동물 ();
        ...
        * */
        ...
        a1.울기();      // 토끼 인스턴스를 동작했기 때문에 실제로는 울기는 재정의한 토끼 에서 실행이 된다.
        a2.울기();
...
//        a1.점프();
//        a2.물기();     // 현재 타입 동물 -> 동물안에는 점프, 물기 라는 메소드가 없다.
...
//        ((호랑이)a1).물어뜯기();     // 토끼는 호랑이가 될 수 없다. runtimeException: 번역은 문제가 없지만, 프로그램 동작 시 오류 발생
        ...
        System.out.println("a1 이 토끼 타입인 지 확인 : " + (a1 instanceof 토끼));
        System.out.println("a1 이 동물 타입인 지 확인: " + (a1 instanceof 동물));  // 다형성 : 여러가지 형태를 가지고 있다
        ...
        *   작성 여부에 따라 명시적, 묵시적 두 가지로 구분된다. */
        ...
        동물 a3 = (동물) new 토끼();     // up-casting 명시적 형변환
                                       // 실제로는 동물이 생략되어 있다 -> 동물이 더 큰 타입 이기 때문에

        동물 a4 = new 토끼();

//        토끼 r3 = a3;                  // down-casting 묵시적 형변환은 안됨.
                                       // a=동물 이여서 동물이 토끼 보다 더 커서

        토끼 r4 = (토끼)a3;           // down-casting 명시적 형변환 -> 강제형변환
